  ...
  useEffect(() => {
    ...
    const fetchGuide = async () => {
      try {
        ...
        if (!isAdmin && !isModerator && !isAuthor) { router.replace('/guides'); return }
        setGuide(g)
        setTitle(g.title)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err.response?.data?.details) {
        ...
        err.response.data.details.forEach((d: any) => { newErrors[d.path?.[0] || 'general'] = d.message })
        setFieldErrors(newErrors)
      }
      setError(err.response?.data?.error || 'Error al actualizar la guía.')
    } finally {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              <Button onClick={handleSubmit} disabled={isSubmitting} className="h-8 text-sm px-5 disabled:opacity-50 gap-2">
                {isSubmitting
